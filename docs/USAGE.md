...
- `--no-signatures`: skip method-signature recovery. The default listing appends a Go-syntax signature (`(_0 []uint8) (int, error)`) to every method whose mtyp resolves; this flag drops back to bare function names. Useful for diff-friendly output or the smallest column width.
