...
/// can parse).
///
/// Pass `types` as an empty slice to emit functions only.
pub fn write_script<W: Write>(
    ...
    match target {
        Target::Ida => write_ida(w, funcs, types),
        Target::Ghidra => write_ghidra(w, funcs, types),
        Target::BinaryNinja => write_binja(w, funcs, types),
    }
}

fn write_ida<W: Write>(w: &mut W, funcs: &[Function], types: &[Type]) -> io::Result<()> {
    writeln!(w, "# Generated by unstrip. Run inside IDA: File -> Script File... (Python3)")?;
    ...
    writeln!(w, "    ida_name.set_name(addr, name, ida_name.SN_FORCE | ida_name.SN_NOCHECK)")?;
    writeln!(w, "    if comment:")?;
    writeln!(w, "        idc.set_func_cmt(addr, comment, 0)")?;
    writeln!(w)?;
    writeln!(w, "def _struct(decl):")?;
    writeln!(w, "    # parse a single C struct decl into IDA's type system")?;
    writeln!(w, "    til = ida_typeinf.get_idati()")?;
    writeln!(w, "    ida_typeinf.parse_decls(til, decl, None, ida_typeinf.PT_TYP)")?;
    writeln!(w)?;

    for f in funcs {
        let name = sanitize_label(&f.name);
        let comment = function_comment(f);
        writeln!(
            w,
            "_func(0x{:x}, {}, {})",
            f.address,
    ...
    if !types.is_empty() {
        writeln!(w)?;
        writeln!(w, "# Recovered struct types ({} total)", count_struct_types(types))?;
...
fn write_ghidra<W: Write>(w: &mut W, funcs: &[Function], types: &[Type]) -> io::Result<()> {
    writeln!(w, "# Generated by unstrip. Run inside Ghidra: Window -> Script Manager (Python).")?;
    writeln!(w, "# @category unstrip")?;
    writeln!(w)?;
    writeln!(w, "from ghidra.program.model.symbol.SourceType import USER_DEFINED")?;
    writeln!(w, "from ghidra.app.util.cparser.C import CParser")?;
    writeln!(w, "from ghidra.program.model.data import DataTypeConflictHandler")?;
    writeln!(w)?;
    ...
    writeln!(w, "        try:")?;
    writeln!(w, "            f = fm.createFunction(name, a, None, USER_DEFINED)")?;
    ...
    writeln!(w, "            dtm.addDataType(parsed, DataTypeConflictHandler.REPLACE_HANDLER)")?;
    writeln!(w, "    except Exception:")?;
    writeln!(w, "        pass")?;
    writeln!(w)?;
...
fn write_binja<W: Write>(w: &mut W, funcs: &[Function], types: &[Type]) -> io::Result<()> {
    writeln!(w, "# Generated by unstrip. Run inside Binary Ninja:")?;
    writeln!(w, "#   bv = binaryninja.load('binary'); exec(open('this.py').read())")?;
    writeln!(w, "from binaryninja import Symbol, SymbolType, Type as BNType")?;
    writeln!(w)?;
    writeln!(w, "def _func(addr, name, comment):")?;
    writeln!(w, "    bv.add_function(addr)")?;
    writeln!(w, "    bv.define_user_symbol(Symbol(SymbolType.FunctionSymbol, addr, name))")?;
    writeln!(w, "    if comment:")?;
...
fn function_comment(f: &Function) -> String {
...
fn struct_c_decls(types: &[Type]) -> Vec<String> {
    ...
    for t in types {
        let KindData::Struct { fields } = &t.kind_data else { continue };
...
fn c_type_for_field(
    ...
    match t.kind {
        KindName::Bool | KindName::Uint8 | KindName::Int8 => ("unsigned char".into(), String::new()),
        KindName::Int16 | KindName::Uint16 => ("unsigned short".into(), String::new()),
        KindName::Int32 | KindName::Uint32 | KindName::Float32 => ("unsigned int".into(), String::new()),
        KindName::Int64 | KindName::Uint64 | KindName::Float64 | KindName::Int | KindName::Uint | KindName::Uintptr => ("unsigned long long".into(), String::new()),
        KindName::Pointer | KindName::UnsafePointer | KindName::Chan | KindName::Map | KindName::Func => ("void *".into(), String::new()),
...
mod tests {
    ...
    fn sanitize_label_handles_go_punctuation() {
        ...
        assert_eq!(sanitize_label("main.Set[go.shape.string]"), "main.Set_go.shape.string_");
